...
class ExtendibleHashTable : public HashTable<K, V> {
 public:
  ...
  class Bucket {
   public:
    ...
    inline auto IsFull() const -> bool { return container_.size() == size_; }

    /** @brief Get the local depth of the bucket. */
    inline auto GetDepth() const -> int { return depth_.load(); }

    /** @brief Increment the local depth of a bucket. */
    inline void IncrementDepth() { depth_.fetch_add(1); }

    inline auto GetItems() -> std::unordered_map<K, V> & { return container_; }
    /**
     *
     * TODO(P1): Add implementation
     *
    ...
    /**
     * @brief Get the RWLatch of the bucket.
     */
    inline auto GetRWLatch() -> std::shared_mutex & { return rwlatch_; };

    /**
     * @brief Get the RWLatch of depth.
     */
    inline auto GetDepthRWLatch() -> std::shared_mutex & { return depth_rwlatch_; };
 ...
 private:
  ...
  mutable std::shared_mutex dir_rwlatch_;     // The rwlatch to protect the directory.
